...
export class AdminComponent implements OnInit {
  ...
  constructor(private korisnikService: KorisnikService, private router: Router, private knjigaService: KnjigaService) { }

  ngOnInit(): void {
    this.korisnikService.dohvatiSveKorisnike().subscribe((data: Korisnik[]) => {
      let sviKorisnici: Korisnik[] = data;
      for (let i = 0; i < sviKorisnici.length; i++) {
       if (sviKorisnici[i].prihvacen == "ceka") {
        this.korisniciCek.push(sviKorisnici[i]);
       }

       if (sviKorisnici[i].prihvacen == "da" && sviKorisnici[i].tip != "administrator") {
        this.korisniciDa.push(sviKorisnici[i]);
       }
      }

      let rr = JSON.parse(localStorage.getItem('rok'));
      if (rr) {
        this.r = rr;
      } else {
        this.r = 14;
  ...
  korisniciCek: Korisnik[] = [];
  korisniciDa: Korisnik[] = [];
  ...
  prihvati(kor: Korisnik) {
    this.korisnikService.prihvacen(kor.kor_ime, "da").subscribe(() => {
      window.location.reload();
    });
  }

  odbaci(kor: Korisnik) {
    this.korisnikService.prihvacen(kor.kor_ime, "ne").subscribe(() => {
      window.location.reload();
    });
  }

  registracija() {
    if (!this.kor_ime || !this.lozinka || !this.lozinka2 || !this.ime || !this.prezime || !this.adresa || !this.telefon || !this.mejl) {
      alert('Niste popunili sva obavezna polja');
      return;
    } 
    if (!this.pwdPattern.test(this.lozinka)) { 
    ...
    if (!this.emailPattern.test(this.mejl)) {
      alert('Neispravni mejl')
      return;
    }
    this.korisnikService.registracija(this.kor_ime, this.lozinka, this.ime, this.prezime, this.adresa, this.telefon, this.mejl, this.url, "da").subscribe(respObj => {
  ...
  selectFile(event: any) { 
		...
		if (mimeType.match(/image\/*/) == null) {
			this.msg = "Samo slike su podržane";
			return;
		}
  ...
  obrisiKorisnika(k: Korisnik) {
    ...
    for (let i = 0; i < k.zaduzenja.length && moze; i++) {
      if (k.zaduzenja[i].datumVracanja == "") {
        moze = false;
        break;
      }
    }
    if (moze) {
      this.korisnikService.obrisiKorisnika(k.kor_ime).subscribe(() => {
        window.location.reload();
      })
    }
    else {
      alert("Korisnik ima knjige na zaduženju");
    }
  }

  azuriranjeKorisnika(k: Korisnik) {
    localStorage.setItem('korisnik', JSON.stringify(k));
    this.router.navigate(['/korisnikpr']);
  }

  obrisiKnjigu(k: Knjiga) {
    let moze: boolean = true
    for (let i = 0; i < this.korisniciDa.length && moze; i++) { 
      for (let j = 0; j < this.korisniciDa[i].zaduzenja.length && moze; j++) {
        if (this.korisniciDa[i].zaduzenja[j].id == k.id && this.korisniciDa[i].zaduzenja[j].datumVracanja == "") {
          moze = false;
          break;
        }
      }
